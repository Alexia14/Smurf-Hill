    ...
    ui(new Ui::View)
{
    ...
    setWindowTitle("Smurf-Hill : The game");
    setWindowIcon(QIcon(":/images/smurfHead"));
    scene->addText(QString("Smurf-Hill : The game"), QFont("Monotype Corsiva", 20));

    QFile file("save.txt");
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox::about(this, "Error", "Backup file not found");
    } else {
...
void View::displayActionHefty() {
    if (!this->menuDisplay) {
        ...
        dial->setWindowIcon(QIcon(":/images/costaud"));
        QGridLayout * l = new QGridLayout();
...
void View::displayActionDoctor() {
    if (!this->menuDisplay) {
        ...
        for (int i=0; i<this->controller->getlistNicePerso()->size(); i++) {
            int treatment = this->controller->getTreatment(i);
            if (treatment > 0) {
                nbSmurfToHeal++;
            }
        }
        ...
        for (int i=0; i<nbSmurfToHeal; i++) {
            ...
            nbBreadSelected->setMaximum(this->controller->getMaxBreadForTreatment(treatment));
            l->addWidget(nbBreadSelected, i+3, 1);
            QSpinBox *nbBerrySelected = new QSpinBox;
            nbBerrySelected->setMinimum(0);
            nbBerrySelected->setMaximum(this->controller->getMaxBerriesForTreatment(treatment));
            l->addWidget(nbBerrySelected, i+3, 2);
            QSpinBox *nbAcornSelected = new QSpinBox;
            nbAcornSelected->setMinimum(0);
            nbAcornSelected->setMaximum(this->controller->getMaxAcornForTreatment(treatment));
                ...
                if (checkbox[i]->isChecked() == true) {
                    if (spinbox[i*3]->value() > this->controller->getNumberBread()) {
                        QMessageBox::warning(this, tr("Erreur"),
                                             tr("Attention, vous n'avez pas assez de pains pour effectuer cette tâche."));
                    } else if (spinbox[(i*3)+1]->value() > this->controller->getNumberBerry()) {
                        QMessageBox::warning(this, tr("Erreur"),
                                             tr("Attention, vous n'avez pas assez de baies pour effectuer cette tâche."));
                    } else if (spinbox[(i*3)+2]->value() > this->controller->getNumberAcorn()) {
                        QMessageBox::warning(this, tr("Erreur"),
                                             tr("Attention, vous n'avez pas assez de glands pour effectuer cette tâche."));
